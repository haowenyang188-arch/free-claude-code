...
Supports local Whisper (cpu/cuda) via Hugging Face transformers.
Install the voice_local extra to enable transcription: uv sync --extra voice_local
"""
...
def transcribe_audio(
    ...
    *,
    ...
    Supports whisper_device="cpu"/"cuda" via local Whisper (requires voice_local extra).

    Args:
        file_path: Path to audio file (OGG, MP3, MP4, WAV, M4A supported)
        mime_type: MIME type of the audio (e.g. "audio/ogg")
        whisper_model: Model ID or short name (local)
        whisper_device: "cpu" | "cuda" (defaults to WHISPER_DEVICE env var)
    ...
    Raises:
        ...
        ImportError: If voice_local extra not installed
    """
