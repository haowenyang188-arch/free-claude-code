...
Keep the root instruction context small. Load the following detailed rules only when the current task actually requires them.

## SOP Workbench

For tasks involving SOP Workbench architecture, roles, routing, workflow state ownership, reviews, handoffs, `ROLE_CAPABILITIES`, `STATUS_AUTHORITY`, `REVIEW_ROUTING`, or related Workbench implementation:

- Read `workbench/docs/codex-role-contract.md` before planning or modifying code.
- Treat that document as frozen architecture unless the user explicitly requests a redesign.

## GitNexus

When using, configuring, debugging, or relying on GitNexus:

- Read `docs/gitnexus-codex-guidance.md` before using GitNexus-specific commands or assumptions.

Do not load this document for unrelated tasks.

## Windows / WSL Paths

For any task involving Windows, WSL, PowerShell, CMD, Git Bash, drive mounts, launch scripts, executables, shortcuts, working directories, or filesystem paths crossing Windows/WSL boundaries:

- Read `docs/windows-wsl-path-safety.md` before executing path-sensitive operations.
- Never guess or silently translate a path.
- Verify environment, cwd, target path, source of the path, and existence before destructive or state-changing operations.

Do not load the full path document for unrelated Linux-only repository work.

## Routing Principle

Detailed documents are conditional context, not default context.

The current user request remains the source of truth. Load only the smallest additional instruction set required for the task.
